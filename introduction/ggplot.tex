...
% \author{Naupaka Zimmerman (@naupakaz) \linebreak \linebreak Hat tip to Karthik Ram (@\_inundata) and Andrew Tredennick (@ATredennick) for slides}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# library("scales")}
\hlcom{# conts <- unique(gapminder[,"continent"])}
\hlcom{# cols <- scales::hue_pal()(length(conts))}
\hlcom{# par(mfrow = c(2,3))}
\hlcom{# counter<-1}
\hlcom{# for (i in conts) \{}
\hlcom{#   plot(gapminder[which(gapminder$continent == i), }
\hlcom{#   			   "gdpPercap"],}
\hlcom{#        gapminder[which(gapminder$continent == i), }
\hlcom{#        		  "lifeExp"], }
\hlcom{#   	   col = cols[counter],}
\hlcom{#        xlab = "Per Capita GDP", }
\hlcom{#		 ylab = "Life Expectancy (yrs)",}
\hlcom{#        main = i, las = 1, log = "x")}
\hlcom{#   fit <- lm(gapminder[which(gapminder$continent == i),}
\hlcom{#   					"lifeExp"] ~ }
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#       log(gapminder[which(gapminder$continent == i),}
\hlcom{#       			  "gdpPercap"]))}
\hlcom{#   pred <- predict(fit, interval = "confidence")}
\hlcom{#   lines(sort(gapminder[which(gapminder$continent == i), }
\hlcom{#   					 "gdpPercap"]), }
\hlcom{#         sort(pred[,1]))}
\hlcom{#   lines(sort(gapminder[which(gapminder$continent == i), }
\hlcom{#   					 "gdpPercap"]), }
\hlcom{#         sort(pred[,2]), lty = 2)}
\hlcom{#   lines(sort(gapminder[which(gapminder$continent == i), }
\hlcom{#   					 "gdpPercap"]),}
\hlcom{#         sort(pred[,3]), lty = 2)}
